...
def iter_local_state_candidates_bk(user_data_dir: str):
    # User Data 直下の Local State バリエーションを総当たり
    ...
    for pat in pats:
        for p in glob.glob(os.path.join(user_data_dir, pat)):
            if os.path.isfile(p) and p not in seen:
                ...
                return p  # まずは最初の候補だけ使う（十分なことが多い）
    return None
...
def _local_state_from_cookie_path_bk(cookie_path: str) -> Optional[str]:
    # ...\User Data\Profile 7\Network\Cookies → ...\User Data\Local State
    ud = os.path.normpath(os.path.join(cookie_path, "..", ".."))
    if os.path.basename(ud).lower() != "user data":
        return None
    ls = os.path.join(ud, "Local State")
    return ls if os.path.exists(ls) else None

def get_browser_key_from_cookie_path_bk(cookie_path: str) -> Optional[bytes]:
    print("cookie_path=",cookie_path)
    local_state_path = _local_state_from_cookie_path(cookie_path)
    if not local_state_path:
        return None
...
def decrypt_cookie_bk3(encrypted_value, master_key, *, debug=False):
    try:
        ...
        if encrypted_value.startswith((b"v10", b"v11", b"v20")):
            ...
            if debug:
                print("[decrypt_cookie] prefix=v20 len(nonce)=", len(nonce),
                      "len(ct)=", len(ct), "len(tag)=", len(tag))
            from Cryptodome.Cipher import AES
            cipher = AES.new(master_key, AES.MODE_GCM, nonce=nonce)  # ← nonceはキーワードで
...
def decrypt_cookie_bk2(encrypted_value: bytes, master_key: bytes):
    try:
        if encrypted_value.startswith((b"v10", b"v11", b"v20")):
            nonce = encrypted_value[3:15]
            payload = encrypted_value[15:]
            ...
            return win32crypt.CryptUnprotectData(encrypted_value, None, None, None, 0)[1].decode("utf-8", errors="strict")
    except Exception:
        return None

def decrypt_cookie_bk(encrypted_value, key):
    try:
        ...
        if encrypted_value[:3] in [b'v10', b'v20']:
            ...
            print("decrypted_bytes.decode PTN1")
            print(decrypted_bytes.decode("utf-8", errors="strict"))

            # CookieはASCII相当のはず。万一非ASCIIでもASCIIで返す（エンコード不可文字は?）
            return decrypted_bytes.decode("utf-8", errors="strict")
        else:
            decrypted_bytes = win32crypt.CryptUnprotectData(encrypted_value, None, None, None, 0)[1]
            print("decrypted_bytes.decode PTN2")
            print(decrypted_bytes.decode("utf-8", errors="strict"))
            return decrypted_bytes.decode("utf-8", errors="strict")
    except Exception as e:
        print("None")
        # 復号失敗時は None を返す（日本語メッセージを混ぜない）
        return None


# ===== 復号 =====
def decrypt_cookie_bk(encrypted_value, key: bytes) -> Optional[str]:
    """
    Chrome/Edge cookies.encrypted_value を復号。
    - 先頭が 'v' のフォーマットは AES-GCM（v10/v20/v30...）
    - それ以外は DPAPI 平文
    復号失敗時は None を返す
    """
    try:
        ...
        if encrypted_value[:1] == b'v':  # v10/v20/v30...
            iv = encrypted_value[3:15]
            payload = encrypted_value[15:]
            if len(payload) < 16:
                return None
...
def get_x_cookies_from_profile_bk(profile_folder: str) -> Tuple[Optional[str], Optional[str], Optional[str]]:
    """
    指定プロファイルから auth_token / ct0 を取得。
    Chrome → 失敗時 Edge を試す。
    encrypted_value 復号失敗時は value 列（平文）も試す。
    最後の保険として browser_cookie3 を使ってドメイン走査。
    戻り値: (auth_token, ct0, error)
    """
#    for browser in ("chrome", "edge"):
    for browser in ("chrome",):
        print("browser in ",browser)
        cookie_path = _cookies_db_path(browser, profile_folder)
        ...
        conn = None
        try:
#            master_key = get_chromium_master_key(profile_folder)
#            print(master_key)
#            print("master_key")
#            key = get_browser_key(browser)

            # cookie_path: ...\User Data\Profile 7\Network\Cookies
#            user_data_dir = os.path.normpath(os.path.join(cookie_path, "..", ".."))  # ...\User Data
#            key = try_keys_until_ok(tmp.name, user_data_dir)
#            if not key:
#                # この User Data 内のどの Local State でも復号できない → コピー由来/別ユーザー由来の可能性大
#                continue
            ...
            ok, ng = quick_probe_any_cookie(tmp.name, key, limit=10)
            print("OK:", ok[:3], "NG:", ng[:3])
            ...
            host_patterns = ["%x.com", "%.x.com", "%twitter.com", "%.twitter.com"]
            names = ("auth_token", "ct0")

            for host_like in host_patterns:
                for name in names:
                    print("host_like=",host_like)
                    print("name=",name)
                    cur.execute(
                        """
                        SELECT encrypted_value, value
                    ...
                    rows = cur.fetchall()
                    for enc, plain in rows:
                        print("enc=",enc," plain=",plain)
                        candidate: Optional[str] = None
                        if enc:
                            candidate = decrypt_cookie(enc, key, debug=True)
                        if candidate is None and plain:
                            candidate = plain  # 平文が入っている場合
    ...
    # ---- 最終フォールバック：browser_cookie3 で x.com / twitter.com を総なめ ----
    try:
        import browser_cookie3
        for fetcher in (browser_cookie3.chrome, browser_cookie3.edge):
            try:
                jar: Dict[str, str] = {}
                # x.com
                cj = fetcher(domain_name="x.com")
                for c in cj:
                    if c.name in ("auth_token", "ct0"):
                        jar[c.name] = c.value
                # twitter.com
                cj2 = fetcher(domain_name="twitter.com")
                for c in cj2:
                    if c.name in ("auth_token", "ct0") and c.value:
                        jar[c.name] = c.value

                a = jar.get("auth_token")
                c = jar.get("ct0")
                if a and c:
                    return a, c, None
            except Exception:
