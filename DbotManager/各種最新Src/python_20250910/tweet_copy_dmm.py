...
# コマンドライン引数の解析関数
def tweet_copy_dmm(credentials , search_row , dmmid):

#    search_row = get_search_list(search_id)
    print("search_row=",search_row)
    print("search_row['search_user_name']=",search_row['search_user_name'])
...
#            api = tweepy.API(auth)
        ...
        for tweet in result_data:
            ...
            # データベースから履歴検索
            search_row = get_search_list(search_row['id'])

            # search_row が None の場合はスキップ
            if search_row is None:
                print(f"search_row is None for search_id: {search_id}")
                continue

#            print("search_row['last_monomane_time']=",search_row['last_monomane_time'])

            # 古いツイートに関しては処理しない
            if 'last_monomane_time' in search_row and search_row['last_monomane_time'] is not None:
#            if search_row['last_monomane_time'] is not NoneType:
                if search_row['last_monomane_time'] >= created_at_str:
                    continue
...
#                outputLog(created_at_str)
                ...
                update_search_list(search_row['id'] , "monomane" , tweet_data['id'] , created_at_str)
