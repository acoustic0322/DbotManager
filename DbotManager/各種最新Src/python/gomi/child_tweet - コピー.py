...
def background_task(tweet_id, like_list, bookmark_list, repost_list, reply_list, reptorep):
    ...
    print(f"[{get_now()}] reptorep: {reptorep}")
    ...
    for account_id in all_account_ids:
        ...
        #     threading.Thread(target=tweet_task, args=(account_id, "like", tweet_id)).start()
                ...
                # like_list があれば mass_like_tweet でまとめて自然分散実行
        if like_list:
            mass_like_tweet(tweet_id, like_list)
            # like処理はここで完結するので、ループ内の個別likeはスキップ
...
def mass_like_tweet(tweet_id, like_list):
"""
like_listのIDを7〜10分で自然に分散してlike実行
"""
if not like_list:
    print(f"[{get_now()}] like_list が空です")
    return 0

target_count = len(like_list)
window_minutes = random.uniform(7.0, 10.0)
total_seconds = window_minutes * 60

base_interval = total_seconds / target_count
min_interval = max(1.2, base_interval * 0.6)
max_interval = base_interval * 1.6

random.shuffle(like_list)

success_count = 0
start_time = time.time()

print(f"[{get_now()}] mass_like_tweet 開始 → {target_count}件 / {window_minutes:.1f}分")

for account_id in like_list:
    elapsed = time.time() - start_time
    remaining = total_seconds - elapsed
    if remaining <= 0:
        break

    current_max = min(max_interval, remaining / (target_count - success_count) * 1.5 if (target_count - success_count) > 1 else max_interval)

    time.sleep(random.uniform(min_interval, current_max))

    threading.Thread(target=tweet_task, args=(account_id, "like", tweet_id)).start()

    success_count += 1
    print(f"[{get_now()}] likeスレッド開始 {success_count}/{target_count} │ ID: {account_id}")

total_time = time.time() - start_time
print(f"[{get_now()}] mass_like_tweet 完了 → {success_count}件開始 / {total_time:.1f}秒")
