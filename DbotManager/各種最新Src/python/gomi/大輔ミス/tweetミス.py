...
from twitter_api_v2 import get_tweet_by_id
...
#        result1 , contents1 = True , "" #未実装
        ...
        # まずツイート取得（見た）API過度に使用するためいったんなし
        # success, tweet_data = get_tweet_by_id(credentials, tweet_id)
        # if success:
            # result1 , contents1 = proc_like_v2(credentials, tweet_id)
            # contents2 = json.dumps(tweet_data)  # ← 文字列に変換
            # result2 = True
        #else:
            #result1 = False
            #contents1 = tweet_data
        result1 , contents1 = proc_like_v2(credentials, tweet_id)
    elif mode == "jap_like":
