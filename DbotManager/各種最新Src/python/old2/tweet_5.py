...
# ツイートにいいねをする関数
def proc_like_tweet_cli(client, tweet_id):
    try:
...
def proc_create_tweet(client ,value):
...
#        client = create_client(credentials)
        client = create_client2(credentials)

        if client:
            if tweet_mode == "tweet":
                proc_create_tweet(client, tweet_text)
            elif tweet_mode == "like":  #動かない
                proc_like_tweet_cli(client, tweet_id)
            elif tweet_mode == "bookmark":
                # API認証を確認
                api = create_api(credentials)
                proc_bookmark_tweet(api, tweet_id)
            else:
