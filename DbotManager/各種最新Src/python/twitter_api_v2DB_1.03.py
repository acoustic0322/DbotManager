...
import re
import random  # ← 大輔追加_12.20(59行目)
import uuid    # ← 大輔追加_12.20
...
# =============================================================================
# いいね・ブクマ・リプ・リツイートの際のランダムヘッダー用定数12.20大輔追加
# =============================================================================
...
USER_AGENTS = [
    ...
    "Mozilla/5.0 (Linux; Android 12; SO-52C) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/120.0.6099.144 Mobile Safari/537.36",
    "Mozilla/5.0 (Linux; Android 12; SOG07) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/119.0.6045.193 Mobile Safari/537.36",
    "Mozilla/5.0 (Linux; Android 13; AQUOS sense7) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/120.0.6099.144 Mobile Safari/537.36",
    # iPad
    ...
    "Mozilla/5.0 (iPad; CPU OS 16_7_2 like Mac OS X) AppleWebKit/605.1.15 (KHTML, like Gecko) Version/16.7.2 Mobile/15E148 Safari/604.1",
    "Mozilla/5.0 (iPad; CPU OS 16_6_1 like Mac OS X) AppleWebKit/605.1.15 (KHTML, like Gecko) Version/16.6.1 Mobile/15E148 Safari/604.1",
...
def get_session_headers(credentials):
    """アカウントIDベースでヘッダー固定（毎回同じ値）"""
    account_id = credentials.get('id', 0)
    ...
    outputLog(f"セッションヘッダー: account_id={account_id}, UA={user_agent[:30]}...")
...
def proc_post_v2(credentials ,comment, reply_to_tweet_id , ai_enable):
    ...
    # ヘッダー（セッション固定）12.20大輔変更ヘッダーランダム化
    session = get_session_headers(credentials)
    headers = {
        "Authorization": f"Bearer {access_token}",
        "Content-Type": "application/json",
        "User-Agent": session["User-Agent"],
        "Accept-Language": session["Accept-Language"],
    }
...
def proc_like_v2(credentials, tweet_id):
    ...
    data = {
     ...
     # ヘッダー（セッション固定）12.20大輔変更(ヘッダをランダム化した)
    session = get_session_headers(credentials)
    headers = {
        "Authorization": f"Bearer {access_token}",
        "Content-Type": "application/json",
