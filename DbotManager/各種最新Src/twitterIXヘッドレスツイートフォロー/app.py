...
def check_ixbrowser_status():
    try:
        from modules.ixbrowser.ixbrowser_controller import IXBrowserController
        controller = IXBrowserController()
        controller.client.get_profile_list(limit=1)
        if controller.client.code == 0:
            return "OK", ""
        else:
            return "ERROR", f"Code {controller.client.code}: {controller.client.message}"
...
def get_db():
    import sys
    import importlib
    if 'modules.mutual_follow.db_manager' in sys.modules:
        importlib.reload(sys.modules['modules.mutual_follow.db_manager'])
    from modules.mutual_follow.db_manager import DBManager
    return DBManager()

def load_accounts(group_id=None):
    df = _load_accounts_cached(group_id)
    if df is not None and not df.empty and 'following_count' not in df.columns:
        st.cache_data.clear()
        df = _load_accounts_cached(group_id)
    return df

@st.cache_data(ttl=60)
def _load_accounts_cached(group_id=None):
    db = get_db()
    ...
    if "editing_account" in st.session_state and st.session_state.editing_account:
        ...
        if not editing_data.empty:
            ...
            with st.container(border=True):
                ...
                base_categories = ["日常生活", "ゲーム", "美容", "子育て", "アイドル", "エンジニア", "アニメ", "料理", "web2", "web用", "非公式用", "その他"]
                ...
                # [NEW] PC Selection in edit overlay
                pc_edit_options = ["未指定"]
                try:
                    if os.path.exists('config.json'):
                        with open('config.json', 'r', encoding='utf-8') as f:
                            cfg = json.load(f)
                            pc_edit_options += sorted(list(set(cfg.get('PC_MAP', {}).values())))
                except: pass
                
                current_assigned_pc = str(acc_data.get('assigned_pc', ''))
                if not current_assigned_pc or current_assigned_pc == 'None' or current_assigned_pc == 'nan':
                    current_assigned_pc = "未指定"
                
                try:
                    pc_index = pc_edit_options.index(current_assigned_pc)
                except ValueError:
                    pc_index = 0
                
                new_assigned_pc = col_g2.selectbox("担当PC", pc_edit_options, index=pc_index, key=f"edit_pc_{editing_name}")
                save_assigned_pc = new_assigned_pc if new_assigned_pc != "未指定" else ""
                ...
                if col_eb1.button("💾 変更を保存", type="primary", use_container_width=True):
                    ...
                    try:
                        cursor.execute(f'''
                            UPDATE accounts 
                            SET username={p}, auth_token={p}, ct0={p}, password={p}, totp_secret={p}, email={p}, display_name={p}, category={p}, group_id={p}, group_name={p}, assigned_pc={p}
                            WHERE username={p}
                        ''', (clean_new_name, new_auth, new_ct0, new_pw, new_totp, new_email, new_display_name, new_category, str(new_group_id or ""), str(new_group_name if new_group_name != "指定なし" else ""), save_assigned_pc, editing_name))
...
with st.sidebar:
    ...
    def nav_item(label, icon, key_suffix):
        ...
        if st.button(f"{icon} {label}", key=f"nav_{key_suffix}", use_container_width=True, type="secondary" if not is_active else "primary"):
            ...
            # 選手権以外のページへ遷移する際、初期表示の全選択判定フラグをリセットする
            if label != "選手権":
                if 'selection_initialized' in st.session_state:
                    del st.session_state.selection_initialized
            st.rerun()
    ...
    # ixBrowser Status Check
    status, detail = check_ixbrowser_status()
    if status == "OK":
        st.sidebar.success("🟢 ixBrowser Connected")
    elif status == "ERROR":
        st.sidebar.error("🔴 ixBrowser Logged Out!")
        st.sidebar.caption("※別のPCでログインしたため、このPCからログアウトされた可能性があります。**ixBrowserアプリを開いて再ログイン**してください。")
    else:
        st.sidebar.warning("🟡 ixBrowser Offline")
        st.sidebar.caption("※ixBrowserアプリが起動していないか、Local APIがオフになっています。**ixBrowserアプリを起動**し、設定でLocal APIをONにしてください。")
...
def render_progress_monitor():
    if os.path.exists('data/task_progress.json'):
        try:
            ...
            """
            st.iframe(src=dashboard_html, height=220)
    ...
    with col_g1:
        if st.button("🚀 全PCに同期指示 (START)", use_container_width=True, type="primary", key="global_start_comp"):
            try:
                ...
                db.reset_all_in_use()
                db.bulk_clear_sync_status("正常|完了|おすすめNG|センシティブ|実行中|ログイン失敗: (False, None)")
            ...
            if i + j < len(account_list):
                ...
                with cols[j]:
                    ...
                    card_html = (
                        ...
                        f'{cat_html}'
                        f'{status_html}'
                        f'<div style="font-size: 0.75rem; color: #888; margin-bottom: 2px;">🕐 {acc.get("last_tweet_at", "未投稿")}</div>'
                        f'<div style="font-size: 0.75rem; color: #aaa; font-weight: 600; margin-bottom: 5px;">📊 {following} / {followers}</div>'
...
if page == "選手権":
    if 'selection_initialized' not in st.session_state:
        ...
        try:
            conn = db.get_connection()
            cursor = conn.cursor()
            cursor.execute("UPDATE accounts SET selected = 1")
            conn.commit()  # SQLiteおよびPostgres両方でコミットを反映
            ...
            if target_group:
                with st.status(f"グループ '{target_group['title']}' からインポート中...", expanded=True) as status:
                    ...
                    if not profiles:
                        ...
                        # --- Pruning: Delete DB accounts that are not in IXBrowser ---
                        ...
                        if target_group and target_group['id'] is not None:
                            group_title = target_group.get('title', '')
                            db_group_accounts = current_df[
                                (current_df['category'] == group_title) | 
                                (current_df['group_name'] == group_title)
                            ]['screen_name'].tolist()
                            to_delete = [name for name in db_group_accounts if name not in current_ix_names]
                        else:
                            ex_titles = [g.get('title') for g in all_ix_groups if g.get('id') in excluded_group_ids]
                            for idx, row in current_df.iterrows():
                                name = row.get('screen_name')
                                cat = row.get('category')
                                gname = row.get('group_name')
                                if name not in current_ix_names and cat not in ex_titles and gname not in ex_titles:
                                    to_delete.append(name)
                        
                        if to_delete:
                            status.write(f"  🗑️ 削除中 (IXBrowser上で削除されたためDBから削除): {', '.join(to_delete)}")
                            for name in to_delete:
                                db.delete_account(name)
                            current_df = current_df[~current_df['screen_name'].isin(to_delete)]
                            existing_names = set(current_df['screen_name'].astype(str).tolist())
        ...
        with st.status("🍪 クッキー同期中...", expanded=True) as sync_status:
            ...
            for idx, row in enumerate(targets):
                ...
                try:
                    ...
                    if not pid:
                        sync_status.write(f"🗑️ [{idx+1}/{total_sync}] @{uname} IXBrowser内にプロファイルがないためDBから削除しました")
    ...
    # 旧カラム構成でキャッシュされている場合はクリアして強制再読込
    if 'cached_df_all' in st.session_state and st.session_state.cached_df_all is not None:
        if 'following_count' not in st.session_state.cached_df_all.columns:
            del st.session_state.cached_df_all
            st.cache_data.clear()
    ...
    # [NEW] PC Filter UI
    pc_options = ["すべて", "未割り当て"]
    try:
        if os.path.exists('config.json'):
            with open('config.json', 'r', encoding='utf-8') as f:
                cfg = json.load(f)
                pc_map = cfg.get('PC_MAP', {})
                pc_options += sorted(list(set(pc_map.values())))
    except: pass
    selected_pc_filter = st.selectbox("💻 担当PCで絞り込み", pc_options, index=0, key="mgmt_pc_filter")
    ...
    # [NEW] PC Filter Logic
    if selected_pc_filter != "すべて":
        if selected_pc_filter == "未割り当て":
            df_display = df_display[(df_display['assigned_pc'].isna()) | (df_display['assigned_pc'].fillna('').str.strip() == '')]
        else:
            df_display = df_display[df_display['assigned_pc'] == selected_pc_filter]

    st.markdown(f"**表示中: {len(df_display)} / 全体: {len(df_all)} アカウント**")
    show_cookie_values = st.checkbox("auth_token / ct0 をカードに表示", value=False, key="mgmt_show_cookie_values_v3")
    ...
    with st.expander("🚀 高度な一括操作 (数千アカウント対応)", expanded=False):
        ...
        # [NEW] PC Auto-redistribute UI and Logic
        st.markdown("---")
        st.markdown("#### 💻 PC自動均等振り分け")
        st.info("生存中（is_alive=1）かつ選択中（Select=True）のアカウントを、config.json に定義された全PCに決定論的かつ均等に自動で振り分けます。")
        
        # Load PC list dynamically
        all_pc_ids = []
        try:
            if os.path.exists('config.json'):
                with open('config.json', 'r', encoding='utf-8') as f:
                    cfg = json.load(f)
                    pc_map = cfg.get('PC_MAP', {})
                    all_pc_ids = sorted(list(set(pc_map.values())))
        except: pass

        if st.button("🤖 選択中アカウントをPCへ自動均等振り分け", use_container_width=True):
            if not all_pc_ids:
                st.error("config.json からPC_MAPを取得できませんでした。")
            else:
                db = DBManager()
                
                # すでに割り当てられているPCのアカウント数を集計
                pc_counts = {pc: 0 for pc in all_pc_ids}
                for _, row in df_all[df_all['is_suspended'] == False].iterrows():
                    apc = str(row.get('assigned_pc', '')).strip()
                    if apc in pc_counts:
                        pc_counts[apc] += 1
                
                # 対象は「選択中かつ生存中」かつ「PC未割り当て」のアカウントのみ
                def is_pc_empty(val):
                    v = str(val).strip()
                    return v in ['', 'None', 'nan'] or not val

                df_targets = df_all[
                    (df_all['Select'] == True) & 
                    (df_all['is_suspended'] == False) & 
                    (df_all['assigned_pc'].apply(is_pc_empty))
                ].sort_values(by='screen_name')
                
                if df_targets.empty:
                    st.warning("対象となる『選択中・生存中かつPC未割り当て』のアカウントがありません。（すべて配置済みです）")
                else:
                    success_count = 0
                    conn = db.get_connection()
                    cursor = conn.cursor()
                    ...
                    try:
                        ...
                        if 'cached_df_all' in st.session_state:
                            del st.session_state.cached_df_all
                        st.success(f"✅ 新規 {success_count} 件のアカウントを既存の割り当てを壊さずに均等に振り分けました！")
                        time.sleep(2)
                        st.rerun()
                    except Exception as e:
                        st.error(f"振り分け保存エラー: {e}")
        st.markdown("---")
    ...
    if col_mgmt_a.button("全て選択", use_container_width=True):
        if not df_display.empty:
            ...
            clear_account_cache()
            # [HIGH-SPEED & SAFE] Session state keys update (KeyErrorを防ぐために値を安全に更新)
            for k in list(st.session_state.keys()):
                if k.startswith("sel_m_"):
        ...
        for j in range(columns_per_row):
            if i + j < len(account_list):
                ...
                with cols[j]:
                    ...
                    auth_value = "" if not has_auth else html.escape(str(acc.get('auth_token', '')).strip())
                    ct0_value = "" if not has_ct0 else html.escape(str(acc.get('ct0', '')).strip())
                    cookie_detail_html = ""
                    if show_cookie_values:
                        empty_html = "<span style='color:#e74c3c;'>EMPTY</span>"
                        cookie_detail_html = f"""
                            <div style="margin-top: 6px; text-align: left; font-size: 0.58rem; line-height: 1.2; color: #aaa; word-break: break-all; background: rgba(0,0,0,0.22); border-radius: 6px; padding: 6px;">
                                <div><span style="color:#D4AF37;">auth_token</span>: {auth_value if auth_value else empty_html}</div>
                                <div style="margin-top: 4px;"><span style="color:#D4AF37;">ct0</span>: {ct0_value if ct0_value else empty_html}</div>
                            </div>
                        """
                    ...
                    # [NEW] PC割り当て状態の確認
                    assigned_pc_val = str(acc.get('assigned_pc', '')).strip()
                    if not assigned_pc_val or assigned_pc_val == 'None' or assigned_pc_val == 'nan':
                        assigned_pc_val = "未割当"
                    pc_badge_color = "#3498db" if assigned_pc_val != "未割当" else "#7f8c8d"
                        ...
                        <div style="background: rgba(255,255,255,0.03); border: 1px solid rgba(212,175,55,0.2); border-radius: 10px; padding: 10px; text-align: center; margin-bottom: 5px;">
                            <div style="display: flex; justify-content: space-between; margin-bottom: 5px;">
                                ...
                                <div style="display: flex; gap: 4px;">
                                    <span style="background: {pc_badge_color}22; color: {pc_badge_color}; font-size: 0.6rem; padding: 1px 4px; border-radius: 4px;">💻 {assigned_pc_val}</span>
                    ...
                    if show_cookie_values and cookie_detail_html:
                        st.markdown(cookie_detail_html, unsafe_allow_html=True)
    ...
    if df_display.empty:
        ...
        if col_act2.button("🧹 一括解除", key="frozen_clr"):
            for _, row in df_display.iterrows():
                uname = row['username']
                db.update_sync_status(uname, "")
                db.update_account_status(uname, is_suspended=False)
            if os.path.exists('data/task_progress.json'):
                try:
                ...
                if i + j < len(account_list):
                    ...
                    with cols[j]:
                        ...
                        if bt1.button("✅", key=f"frozen_cl_{name}", help="解除"):
                            db.update_sync_status(acc.get('username'), "")
                            db.update_account_status(acc.get('username'), is_suspended=False)
                            if os.path.exists('data/task_progress.json'):
                                try:
                                    with open('data/task_progress.json', 'r', encoding='utf-8') as f: p_data = json.load(f)
    ...
    if df_display.empty:
        ...
        if col_act2.button("🧹 一括解除", key="fail_clr"):
            ...
            st.cache_data.clear(); st.rerun()
        if col_act3.button("🚀 一括再ログイン実行", key="exec_fail_bulk_retry", type="primary"):
            for _, row in df_display.iterrows():
                db.update_sync_status(row['username'], "実行失敗 | BULK_RELOGIN")
            script_path = os.path.abspath('bulk_login.py')
            cmd = [sys.executable, script_path]
            proc = subprocess.Popen(
                ['cmd', '/k'] + cmd if sys.platform == 'win32' else cmd,
                cwd=os.getcwd(),
                creationflags=subprocess.CREATE_NEW_CONSOLE if sys.platform == 'win32' else 0
            )
            st.caption(f"bulk_login.py PID: {proc.pid}")
            st.info("実行失敗アカウントの一括再ログインを開始しました。別ウィンドウで進行します。")

        if col_act4.button("🗑️ 一括削除", key="exec_fail_bulk_del"):
    ...
    db = DBManager()
    df_all = load_accounts()
    df_display = df_all[df_all['sync_status'].str.contains('ログイン失敗', na=False)]
    ...
    if df_display.empty:
        ...
        show_login_fail_cookie_values = st.checkbox("auth_token / ct0 を表示", value=False, key="login_fail_show_cookie_values")
        col_act1, col_act2, col_act3, col_act4 = st.columns(4)
            ...
            for j in range(columns_per_row):
                if i + j < len(account_list):
                    ...
                    with cols[j]:
                        ...
                        if show_login_fail_cookie_values:
                            auth_raw = str(acc.get('auth_token', '')).strip()
                            ct0_raw = str(acc.get('ct0', '')).strip()
                            auth_display = html.escape(auth_raw) if auth_raw and auth_raw != 'nan' else "<span style='color:#e74c3c;'>EMPTY</span>"
                            ct0_display = html.escape(ct0_raw) if ct0_raw and ct0_raw != 'nan' else "<span style='color:#e74c3c;'>EMPTY</span>"
                            st.markdown(
                                f"""
                                <div style="margin: 6px 0; text-align: left; font-size: 0.58rem; line-height: 1.2; color: #aaa; word-break: break-all; background: rgba(0,0,0,0.22); border-radius: 6px; padding: 6px;">
                                    <div><span style="color:#D4AF37;">auth_token</span>: {auth_display}</div>
                                    <div style="margin-top: 4px;"><span style="color:#D4AF37;">ct0</span>: {ct0_display}</div>
                                </div>
                                """,
                                unsafe_allow_html=True,
                            )
