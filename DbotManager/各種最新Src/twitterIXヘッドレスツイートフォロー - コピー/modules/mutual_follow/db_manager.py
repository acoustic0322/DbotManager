...
class DBManager:
        ...
        if DBManager._shared_conn:
            try:
                if self.db_type == "postgres":
                    ...
                    with DBManager._shared_conn.cursor() as cur:
                        cur.execute("SELECT 1")
                elif self.db_type == "mysql":
                    DBManager._shared_conn.ping(reconnect=False)
            except Exception:
                logger.warning("Shared connection is dead. Reconnecting...")
    ...
    def get_connection_dbot(self):
        ...
        if DBManager._dbot_conn:
            try:
                DBManager._dbot_conn.ping(reconnect=False)
                return DBManager._dbot_conn
            except:
                DBManager._dbot_conn = None
    ...
    def get_all_accounts_df(self):
        ...
        expected_cols = [
            ...
            'group_name', 'auth_token', 'ct0', 'password', 'email', 'totp_secret', 'profile_id', 'assigned_pc',
    ...
    def save_accounts_df(self, df):
        ...
        try:
            ...
            # 保存対象のカラム定義
            core_cols = ['auth_token', 'ct0', 'password', 'email', 'totp_secret', 'profile_id', 'group_id', 'group_name', 'category', 'display_name', 'assigned_pc']
    ...
    def save_accounts_df_for_dbot(self, df):
        ...
        try:
            ...
            # 保存対象のカラム定義
#            core_cols = ['auth_token', 'ct0', 'password', 'email', 'totp_secret', 'profile_id', 'group_id', 'group_name', 'category', 'display_name', 'assigned_pc']
            core_cols = ['auth_token', 'ct0', 'login_password', 'email', 'totp_secret', 'twitter_user_id', 'group_id', 'group_name', 'category', 'display_name', 'assigned_pc']
            existing_core_cols = [c for c in core_cols if c in df.columns]
...
#            all_cols = ['username', 'selected', 'is_alive'] + existing_core_cols
    ...
    def update_sync_status(self, username, status, retry=True):
        try:
            ...
            if retry:
                ...
                DBManager._shared_conn = None # 強制的に再接続させる
    ...
    def get_next_account(self, target_usernames=None):
        try:
            ...
            if target_usernames is not None:
                ...
                """
                params = tuple(target_usernames) + (self.pc_name,)
            else:
                query = f"""
                    SELECT * FROM accounts 
    ...
    def update_cookies(self, username, auth_token, ct0):
        try:
            conn = self.get_connection()
            cursor = conn.cursor()
            p = self._placeholder()
            cursor.execute(f"UPDATE accounts SET auth_token={p}, ct0={p} WHERE username={p}", (auth_token, ct0, username))
            if self.db_type == "postgres": conn.commit()
        except Exception as e:
