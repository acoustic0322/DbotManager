...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $exePath = "C:\\DbotManager\\DbotManager.exe";
    $psexecPath = "C:\\PsTool\\PsExec.exe";

    /*
    $args = sprintf(
        ' UserId=%d TweetID=%s Like=%s Bookmark=%s Reply=%s Repost=%s LikeCount=%d BookmarkCount=%d ReplyCount=%d RepostCount=%d Duplicate=True',
        $current_userid,
        $tweetId,
        $likeEnable,
        $bookmarkEnable,
...
//    $command = "\"$psexecPath\" -accepteula -s -i 2 -d \"$exePath\" $args";
    $command = "\"$psexecPath\" -accepteula -s -i 2 -d \"$exePath\" $args 2>&1";
    */
    ...
    $args = sprintf(
        ...
        $current_userid,
        $tweetId,
        $likeEnable ? 'True' : 'False', // True/False の値を設定
        $bookmarkEnable ? 'True' : 'False',
        $replyEnable ? 'True' : 'False',
        $repostEnable ? 'True' : 'False',
        $likeCount,
        $bookmarkCount,
        $replyCount,
        $repostCount
    ...
    // コマンドをエスケープして実行
//    $command = sprintf('"%s" -accepteula -s -i 2 -d "%s" %s 2>&1', $psexecPath, $exePath, escapeshellarg($args));
    $command = sprintf('"%s" -accepteula -s -i 2 -d "%s" UserId=2 2>&1', $psexecPath, $exePath);
//    $command = sprintf('"%s" -accepteula -s -i 2 -d "%s" 2>&1', $psexecPath, $exePath );
