...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($type == 'user_del') {
        ...
        if ($count > 0) {
            ...
            // ユーザーをデータベースに登録
            $stmt = $conn->prepare("INSERT INTO account_master (user_id, name, login_id, login_password, client_id, client_secret, api_key, api_key_secret, access_token,  access_token_secret, bearer_token, refresh_token, paid , like_enable , bookmark_enable , repost_enable , poset_enable , paid_like , paid_bookmark ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
            $stmt->bind_param("ssssssssssssssssss", $current_userid, $new_name, $new_login_id, $new_login_pass, $new_client_id, $new_client_secret, $new_api_key, $new_api_key_secret, $new_access_token, $new_access_secret, $new_bearer_token, $new_refresh_token ,$new_pad, $new_bookmark_enable , $new_repost_enable , $new_poset_enable , $new_paid_like , $new_paid_bookmark );
            $stmt->execute();
            $stmt->close();
            $conn->close();
