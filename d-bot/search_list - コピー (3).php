...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($type === 'update') {
        // 送信されたデータを取得
        $id = $_POST['id'];
        $time_enable = isset($_POST["time_enable"]) && $_POST["time_enable"] == "1" ? 1 : 0;
...
<!--    <form method="POST" action="?" class="registration-form">  -->
    ...
    function updateHiddenValue(input) {
        let form = input.closest("tr").querySelector(".button_form");
        let name = input.name;
    ...
    function updateHiddenCheckbox(checkbox) {
        let hiddenInput = checkbox.closest("tr").querySelector('input[name="time_enable"]');
        if (hiddenInput) {
