...
for screen_name in screen_names:
    ...
    if os.path.exists(os.path.join(folder, f"{path_tmp_screen_name}_sinceid2.txt")):
        with open(os.path.join(folder, f"{path_tmp_screen_name}_sinceid2.txt"), 'r', encoding='utf-8') as file:  # UTF-8エンコーディングを使用
    ...
    if tweets.data is not None and len(tweets.data) > 0:
        with open(os.path.join(folder, f"{path_tmp_screen_name}_sinceid2.txt"), 'w', encoding='utf-8') as file:
            for tweet in tweets.data:
                file.write(f"{tweet.id}\n")
        for tweet in tweets.data:
            print(f"{tweet.id}:{tweet.text}:{tweet.created_at}")
