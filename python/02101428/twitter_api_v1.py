    ...
    try:
        ...
        try:
            ...
            if 'attachments' in tweet_data and 'media_keys' in tweet_data['attachments']:
                ...
                if tweet_data['type'] == 'photo':
                    outputLog(f"Image URL: {tweet_data['url']}")
                    file = download_url(tweet_data['type'],tweet_data['url'])
                    if os.path.isfile(file):
                        media_files.append(file)
                    else:
                        outputLog("Image URL not found.")

                elif tweet_data['type'] == 'video':
                    outputLog(f"Video URL: {tweet_data['url']}")
