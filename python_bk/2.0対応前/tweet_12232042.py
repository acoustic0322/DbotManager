...
# ツイートにいいねをする関数
def proc_like_oauth20_2(credentials, tweet_id):
    ...
    user_id = get_twitter_useid(new_bearer_token)

    # アクセストークン（OAuth 2.0 Bearer Token）
    access_token = new_bearer_token

    # API エンドポイントとヘッダー
    like_endpoint = "https://api.twitter.com/2/users/{user_id}/likes"  # {user_id} を適切な値に置き換える
    headers = {
        "Authorization": f"Bearer {access_token}",
    ...
    # いいねするターゲットツイートの ID
#    tweet_id = "TARGET_TWEET_ID"  # 実際のツイート ID を指定
    ...
    # HTTP POSTリクエストの送信
    response = requests.post(like_endpoint, headers=headers, json=payload)
    ...
    if response.status_code == 200:
        ...
        print(f"いいねに失敗しました。ステータスコード: {response.status_code}")
...
def like_tweet(credentials, tweet_id, account_id):
...
#    outputLog(f"new_refresh_token= {new_refresh_token}")
    ...
    headers = {"Authorization": f"Bearer {get_access_token(client_id,client_secret)}"}
    payload = {"tweet_id": tweet_id}
    response = requests.post(url, json=payload, headers=headers)

    if response.status_code == 200:
        print("ツイートを「いいね」しました！")
    else:
...
def get_user_id(access_token, username):
    ...
    if response.status_code == 200:
        ...
        print(f"{username}のユーザーID: {user_id}")
        return user_id
    else:
        print(f"エラー: {response.status_code}, {response.text}")
        return None
...
def check_access_token(access_token):
    ...
    response = requests.get(url, headers=headers)
    if response.status_code == 200:
        print("認証成功: User Context アクセストークンを使用しています")
        print("レスポンス:", response.json())
    elif response.status_code == 403:
        print("認証失敗: Application-Only アクセストークンを使用している可能性があります")
    else:
        print(f"エラー: {response.status_code}, {response.json()}")

## 使用中のアクセストークンを入力
#check_access_token("YOUR_ACCESS_TOKEN")
...
def like_tweet3(credentials, tweet_id):
...
# ツイートをブックマークに追加する関数
def proc_like_test(credentials, tweet_id, account_id):
    try:
        client_id = credentials['client_id']
        client_secret = credentials['client_secret']
        refresh_token = credentials['refresh_token']
...
def get_access_token(client_id, client_secret):
    ...
    # クライアントIDとクライアントシークレットをエンコードしてヘッダーに追加
    ...
    # クライアントクレデンシャルフローでトークンを取得
    headers = {
        "Content-Type": "application/x-www-form-urlencoded",
        "Authorization": f"Basic {client_credentials_base64}",  # Basic認証のヘッダー
    }
    data = {
        "grant_type": "client_credentials",  # クライアントクレデンシャルフロー

        "client_type": "service_client",  # ここを追加     

        "client_id": client_id,
        "client_secret": client_secret,  # 必須  
#        "client_type": "public",  # ここを追加     
        "scope": "tweet.read tweet.write users.read",  # 必要なスコープを指定 
    }
    response = requests.post(
        TOKEN_URL,
        headers=headers,
#        auth=(client_id, client_secret),  # Basic認証でクライアントIDとシークレットを送信
        data=data,
    )
    if response.status_code == 200:
...
def refresh_access_token2(credentials , account_id):
...
#    account_id = credentials['id']
    ...
    print('refresh_access_token:',response_json)  # デバッグ用出力
    # print("access_token："+response_json.get('access_token'))
    # print("refresh_token："+response_json.get('refresh_token'))
    # input()

    update_refresh_token(account_id, response_json.get('access_token') , response_json.get('refresh_token'))

#    credentials['bearer_token'] = response_json.get('access_token')
#    credentials['refresh_token'] = response_json.get('refresh_token')
#    return credentials

    return response_json.get('access_token') , response_json.get('refresh_token')

# リフレッシュトークンで新たなベアラトークンを取得
def refresh_access_token3(credentials , account_id):
...
#    account_id = credentials['id']
    ...
    if response.status_code == 200:
        update_refresh_token(account_id, response.json() , refresh_token)
        return response.json()
    else:
        print(f"refresh_access_token3エラー: {response.status_code}, {response.text}")
        return None    
...
#                success , error_log = like_tweet2(credentials, tweet_id)
                success , error_log = test_tweet(credentials)
#                success , error_log = True , ""
            elif mode == "like_oauth20":
                check_bearer_token(credentials)
                success , error_log = proc_like_oauth20_2(credentials, tweet_id)
            elif mode == "bookmark":
